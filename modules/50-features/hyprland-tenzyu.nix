{
  flake.effects.hyprland-tenzyu.requires = [
    "hyprland-core"
    "tenzyu-desktop"
  ];
  ...
  flake.modules.homeManager.hyprland-tenzyu = {
    ...
    classRule = class: effects: {match.class = class;} // effects;

    titleRule = title: effects: {match.title = title;} // effects;
    ...
    emptyXwaylandFloatingRule = effects:
      {
        match = {
          class = "^$";
          title = "^$";
    ...
    isNixos = config.local.hm.context == "nixos";
  in {
    wayland.windowManager.hyprland = {
      ...
      systemd.enable = lib.mkIf (config.local.hm.context == "standalone") true;
      ...
      settings = {
        ...
        window_rule = [
          ...
          (parsecRule {immediate = true;})
          (parsecRule {fullscreen = true;})
          (parsecRule {stay_focused = true;})
          (parsecRule {no_initial_focus = true;})
          (parsecRule {allows_input = true;})
          (parsecRule {focus_on_activate = true;})
          (emptyXwaylandFloatingRule {no_focus = true;})
          (emptyXwaylandFloatingRule {no_follow_mouse = true;})
        ];
      };
    };
